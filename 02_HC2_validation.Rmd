...
# Prepare data for ANOVA: aggregate T_SInf per cell of the full experimental grid
...
# Fit full factorial ANOVA: T_SInf ~ N * lambda * beta * sigma
# Main effects and all interactions
anova_sinf_full <- aov(mean_T_sinf ~ N_factor * lambda_factor * beta_factor * sigma_factor, 
                       data = sinf_anova_data)
anova_summary_full <- summary(anova_sinf_full)

cat("Full Factorial ANOVA: T_SInf ~ N × λ × β_x × σ0\n")
cat("=" |> rep(70) |> paste(collapse = ""), "\n\n")
print(anova_summary_full)

# Extract main effects
cat("\n\nMain Effects Summary:\n")
cat("-" |> rep(50) |> paste(collapse = ""), "\n")

main_effects <- c("N_factor", "lambda_factor", "beta_factor", "sigma_factor")
for (effect in main_effects) {
  if (effect %in% rownames(anova_summary_full[[1]])) {
    row <- anova_summary_full[[1]][effect, ]
    sig <- ifelse(row$`Pr(>F)` < 0.001, "***", 
                  ifelse(row$`Pr(>F)` < 0.01, "**",
                         ifelse(row$`Pr(>F)` < 0.05, "*", "ns")))
    cat(sprintf("  %-20s F = %8.2f, p = %.2e %s\n", 
                effect, row$`F value`, row$`Pr(>F)`, sig))
  }
}

# Key interaction: N × lambda
if ("N_factor:lambda_factor" %in% rownames(anova_summary_full[[1]])) {
  row <- anova_summary_full[[1]]["N_factor:lambda_factor", ]
  cat(sprintf("\n  N × λ interaction:   F = %8.2f, p = %.2e\n", 
...
**Interpretation:**
- **N (sample size)**: Expected to be highly significant—$T_{S_{\mathrm{Inf}}}$ scales with $\sqrt{N}$ by construction.
- **λ (heteroskedasticity)**: Expected to be highly significant—stronger misspecification increases the score.
- **β_x (effect size)**: May or may not be significant; the score measures SE ratio, not effect magnitude.
- **σ0 (baseline noise)**: Should be non-significant if the score is properly normalized.
- **N × λ interaction**: Critical test—if significant, confirms that the N-dependence varies with misspecification level (i.e., the score conflates detection power with severity).

```{r n-dependence-conclusion}
# Final assessment
n_effect <- anova_summary_full[[1]]["N_factor", ]
lambda_effect <- anova_summary_full[[1]]["lambda_factor", ]

cat("\nConclusion:\n")
if (n_effect$`Pr(>F)` < 0.001) {
  cat("→ N effect is HIGHLY SIGNIFICANT (p < 0.001)\n")
  cat("→ T_SInf systematically depends on sample size.\n")
}
if (lambda_effect$`Pr(>F)` < 0.001) {
  cat("→ λ effect is HIGHLY SIGNIFICANT (p < 0.001)\n")
  cat("→ T_SInf responds to heteroskedasticity level (as expected).\n")
}
cat("\n→ Because T_SInf depends on N, it cannot serve as a direct\n")
cat("   measure of misspecification SEVERITY across different sample sizes.\n")
```
