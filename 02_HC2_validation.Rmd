...
# Set seed for reproducibility of sensitivity analysis
set.seed(2024)
...
run_benchmark <- function(N, lambda) {
  reps <- replicate(n_reps_bench, {
    ...
    # S_Inf Test (Manual Calculation to ensure consistency)
    # T_SInf = sqrt(N) * (SE_Robust / SE_Classic - 1)
    se_cls <- summary(fit)$coefficients["x", "Std. Error"]
    se_rob <- sqrt(diag(sandwich::vcovHC(fit, type = best_hc)))["x"]
    t_sinf <- sqrt(N) * (se_rob / se_cls - 1)
    
    c(
      reject_bp = bp_pval < 0.05,
      reject_white = white_pval < 0.05,
      reject_sinf = t_sinf > 1.645  # One-sided test using Z-cutoff
    )
  })
  rowMeans(reps)
}
...
benchmark_res[, c("reject_bp", "reject_white", "reject_sinf")] <- NA

for(i in 1:nrow(benchmark_res)) {
  res <- run_benchmark(benchmark_res$N[i], benchmark_res$lambda[i])
  benchmark_res[i, c("reject_bp", "reject_white", "reject_sinf")] <- res
}
...
ggplot(bench_long, aes(x = lambda, y = Power, color = Test)) +
  ...
  labs(title = "Power Comparison: S_Inf vs Standard Tests",
       subtitle = "S_Inf performs comparably to Breusch-Pagan and White tests",
