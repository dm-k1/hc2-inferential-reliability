...
# Classical coverage summary
cat("\n\nClassical CI Coverage Summary:\n")
cat("-" |> rep(40) |> paste(collapse = ""), "\n")
cat(sprintf("Mean coverage:            %.1f%%\n", mean(hetero_sim_results$coverage) * 100))
cat(sprintf("Min coverage (by cell):   %.1f%%\n", 
            hetero_sim_results[, .(cov = mean(coverage)), by = .(N, hetero_strength)][, min(cov)] * 100))
cat(sprintf("Max coverage (by cell):   %.1f%%\n", 
            hetero_sim_results[, .(cov = mean(coverage)), by = .(N, hetero_strength)][, max(cov)] * 100))
```
...
### Inferential Score vs Sample Size

We examine how the inferential score varies with sample size for different heteroskedasticity levels.

```{r sinf-by-n, fig.width=10, fig.height=5}
# Aggregate S_Inf by N and hetero_strength
sinf_summary <- hetero_sim_results[, .(
  mean_sinf = mean(sr_inf),
  sd_sinf = sd(sr_inf),
  q05 = quantile(sr_inf, 0.05),
  q95 = quantile(sr_inf, 0.95)
), by = .(N, hetero_strength)]

# Plot mean S_Inf with confidence bands
ggplot(sinf_summary[hetero_strength %in% c(0, 0.5, 1.0, 1.5, 2.0)], 
       aes(x = N, y = mean_sinf, color = as.factor(hetero_strength))) +
  geom_ribbon(aes(ymin = q05, ymax = q95, fill = as.factor(hetero_strength)), 
              alpha = 0.2, color = NA) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  ...
  scale_x_log10() +
  labs(
    title = "Inferential Score by Sample Size and Heteroskedasticity",
    subtitle = "Lines show mean; bands show 5th-95th percentile range",
...
### Classical Coverage Degradation

We visualize how classical CI coverage degrades with increasing heteroskedasticity.
...
**Key Observations:**
- Under the null ($\lambda = 0$), $S_{\mathrm{Inf}}$ is centered near zero and classical coverage is near 95%.
- As heteroskedasticity increases, $S_{\mathrm{Inf}}$ shifts positive (robust SE > classic SE) and classical coverage degrades.
- The relationship between $S_{\mathrm{Inf}}$ and coverage loss motivates using this score as a diagnostic for inference reliability.
...
## 4.2 Formal Test of N-Dependence (Multilevel ANOVA)

To formally establish that $T_{S_{\mathrm{Inf}}}$ depends on sample size, we fit a multilevel ANOVA with N as a fixed factor and heteroskedasticity strength as a blocking factor.

```{r n-dependence-anova}
# Prepare data for ANOVA: aggregate T_SInf per (N, hetero_strength) cell
sinf_anova_data <- hetero_sim_results[, .(
  mean_T_sinf = mean(T_SInf)
), by = .(N, hetero_strength)]

# Convert N to factor for ANOVA
sinf_anova_data[, N_factor := as.factor(N)]
sinf_anova_data[, lambda_factor := as.factor(hetero_strength)]

# Fit two-way ANOVA: T_SInf ~ N + lambda
anova_sinf <- aov(mean_T_sinf ~ N_factor + lambda_factor, data = sinf_anova_data)
anova_summary <- summary(anova_sinf)

cat("ANOVA: T_SInf ~ N + Heteroskedasticity Strength\n")
cat("=" |> rep(60) |> paste(collapse = ""), "\n\n")
print(anova_summary)

# Extract F-statistic and p-value for N effect
n_effect <- anova_summary[[1]]["N_factor", ]
cat(sprintf("\nN Effect: F = %.2f, p = %.2e\n", n_effect$`F value`, n_effect$`Pr(>F)`))

if (n_effect$`Pr(>F)` < 0.001) {
  cat("→ Highly significant N-dependence (p < 0.001)\n")
  cat("→ T_SInf is NOT suitable as a direct measure of misspecification severity.\n")
}
```
