...
### Scaled Inferential Score vs Sample Size

We examine how the scaled inferential score $S_{\mathrm{Inf}}^{*} = \sqrt{N} \cdot S_{\mathrm{Inf}}$ varies with sample size for different heteroskedasticity levels.

```{r sinf-by-n, fig.width=10, fig.height=5}
# Aggregate S_Inf_adj by N and hetero_strength
sinf_summary <- hetero_sim_results[, .(
  mean_sinf_adj = mean(sr_inf_adj),
  sd_sinf_adj = sd(sr_inf_adj),
  q05 = quantile(sr_inf_adj, 0.05),
  q95 = quantile(sr_inf_adj, 0.95)
), by = .(N, hetero_strength)]

# Plot mean S_Inf_adj with confidence bands
ggplot(sinf_summary[hetero_strength %in% c(0, 0.5, 1.0, 1.5, 2.0)], 
       aes(x = N, y = mean_sinf_adj, color = as.factor(hetero_strength))) +
  ...
  scale_x_log10() +
  labs(
    title = "Scaled Inferential Score by Sample Size and Heteroskedasticity",
    subtitle = "Lines show mean; bands show 5th-95th percentile range",
    x = "Sample Size (log scale)",
    y = expression(S[Inf]^"*" == sqrt(N) %.% S[Inf]),
    color = "Lambda",
    fill = "Lambda"
  ) +
  theme_minimal()
