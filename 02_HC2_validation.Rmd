...
cat(sprintf("We adopt c = %.4f from the canonical simple regression case.\n", c_empirical))
```
...
# Use centralized plotting function from R/60_tables_and_plots.R
print(plot_scaling_comparison(scaling_long))
```

## 5.6 Formal ANOVA Test: Raw Ratio vs Adjusted Scores

We formally test N-dependence for each metric. We expect the N-Effect ($\eta^2$) to be negligible for the Reliability Score.

```{r anova-comparison}
# ANOVA on Raw Ratio
anova_raw <- aov(sr_ratio ~ N_factor * lambda_factor * beta_factor * sigma_factor, 
                 data = hetero_sim_results)
ss_raw <- summary(anova_raw)[[1]]
ss_total_raw <- sum(ss_raw$`Sum Sq`)
n_eta_raw <- ss_raw["N_factor", "Sum Sq"] / ss_total_raw
lambda_eta_raw <- ss_raw["lambda_factor", "Sum Sq"] / ss_total_raw

# ANOVA on Reliability Score
...
n_eta_adj <- ss_adj["N_factor", "Sum Sq"] / ss_total_adj
lambda_eta_adj <- ss_adj["lambda_factor", "Sum Sq"] / ss_total_adj

cat("ANOVA Effect Size Comparison (η² for N-Factor)\n")
cat("=" |> rep(60) |> paste(collapse = ""), "\n\n")

cat(sprintf("%-30s %12s %12s\n", "Metric", "N Effect", "λ Effect"))
cat("-" |> rep(60) |> paste(collapse = ""), "\n")
cat(sprintf("%-30s %11.4f%% %11.4f%%\n", 
            "T_SInf (Inferential)", n_eta * 100, lambda_eta * 100))
cat(sprintf("%-30s %11.4f%% %11.4f%%\n", 
            "Raw Ratio", n_eta_raw * 100, lambda_eta_raw * 100))
cat(sprintf("%-30s %11.4f%% %11.4f%%\n", 
            sprintf("Reliability Score (c=%.2f)", c_empirical), 
            n_eta_adj * 100, lambda_eta_adj * 100))

cat("\n")
if (n_eta_adj < 0.01) {
  cat("SUCCESS: Reliability Score N-dependence is NEGLIGIBLE (η² < 1%).\n")
  cat(sprintf("The conservative correction (c = %.3f) achieves approximate N-invariance.\n", c_empirical))
} else if (n_eta_adj < 0.06) {
  cat("Reliability Score N-effect is SMALL (η² < 6%).\n")
}
```

## 5.7 Classical Coverage Degradation
...
We benchmark the detection capability of our scores against standard tests (Breusch-Pagan and White). Note: For detection (hypothesis testing), we use the Inferential Score ($S_{\mathrm{Inf}}$), which scales with $\sqrt{N}$.
...
# Use centralized pipeline function from R/10_dgp_and_fits.R
# Note: Global seed propagates via furrr_options(seed = TRUE)
benchmark_res <- run_benchmark_simulations(
  N_grid = N_bench,
  lambda_grid = lambda_bench,
  n_reps = n_reps_bench,
  hc_type = best_hc,
  verbose = TRUE
)
...
# Use centralized plotting function from R/60_tables_and_plots.R
print(plot_benchmark_comparison(bench_long))
```
...
# 1. Aggregate basic metrics from simulation
aggregated_hetero <- aggregate_hetero_sims(hetero_sim_results)

# 2. EXPLICITLY aggregate the calibrated Reliability Score
# This overwrites any default calculation in the helper function to ensure
# we use the c_empirical derived in this notebook.
ratio_agg <- hetero_sim_results[, .(
  mean_sr_ratio_adj = mean(sr_ratio_adj, na.rm = TRUE)
), by = .(N, hetero_strength, beta_x, sigma0)]

# Merge back into main aggregation
aggregated_hetero <- merge(aggregated_hetero, ratio_agg, 
...
cat(sprintf("Aggregated to %d rows using calibrated c = %.4f\n\n", 
            nrow(aggregated_hetero), c_empirical))
```
...
# Use centralized plotting function from R/60_tables_and_plots.R
print(plot_universal_lookup(universal_lookup, hc_type = best_hc))
```
...
1.  **Safety by Design**: We calibrated the Reliability Score on the canonical simple regression case ($p=2$), deriving a correction factor $c \approx$ `r round(c_empirical, 2)`. Sensitivity analysis confirms this is a **conservative** choice—higher-dimensional models typically exhibit lower finite-sample bias ratios, meaning our tool will not under-correct in complex settings.

2.  **N-Invariance**: The calibrated Reliability Score $S_{\mathrm{Rel}}$ removes the finite-sample drift observed in raw metrics ($\eta^2$ for N drops to $<1\%$).

3.  **Detection Power**: The inferential score $S_{\mathrm{Inf}}$ (based on `r best_hc`) has high power to detect heteroskedasticity, comparable to standard tests like Breusch-Pagan and White.

4.  **Universal Mapping**: The final lookup table maps $S_{\mathrm{Rel}}$ directly to expected coverage loss, allowing applied researchers to diagnose reliability using a single, sample-size-independent metric.
