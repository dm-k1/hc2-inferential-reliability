...
#'
#' Creates a three-panel visualization:
#' 1. Overlaid comparison showing both raw and adjusted metrics
#' 2. Spread reduction by lambda (bar chart)
#' 3. Slope comparison across sample sizes
#'
...
plot_scaling_comparison <- function(scaling_long) {
  ...
  # Calculate spread by lambda for each metric
  spread_data <- scaling_long[, .(
    spread = max(Value) - min(Value)
  ), by = .(hetero_strength, Metric)]
  
  spread_wide <- dcast(spread_data, hetero_strength ~ Metric, value.var = "spread")
  spread_wide <- as.data.table(spread_wide)
  
  # Rename columns to simpler names (handle variable Reliability Score name)
  col_names <- names(spread_wide)
  raw_col <- col_names[grepl("Raw", col_names)]
  adj_col <- col_names[grepl("Reliability", col_names)]
  setnames(spread_wide, old = c(raw_col, adj_col), new = c("Raw", "Adjusted"))
  
  spread_wide[, reduction_pct := (Raw - Adjusted) / Raw * 100]
  
  # Calculate slopes (change per log-unit of N) for each lambda
  slope_data <- scaling_long[, {
    if (.N > 1) {
      log_N <- log10(N)
      fit <- lm(Value ~ log_N)
      .(slope = coef(fit)[2])
  ...
  # Panel 1: Overlaid lines showing both metrics
  p1 <- ggplot(scaling_long, 
               aes(x = N, y = Value, 
                   color = as.factor(hetero_strength),
                   linetype = Metric)) +
    geom_line(size = 0.7) +
    geom_point(size = 1.2, alpha = 0.6) +
    scale_x_log10() +
    scale_color_viridis_d(option = "plasma", end = 0.9, name = expression(lambda)) +
    scale_linetype_manual(values = c("Raw Ratio" = "solid", 
                                     "Reliability Score (c=0.226)" = "dashed",
                                     "Reliability Score (c=0.238)" = "dashed")) +
    labs(
      title = "Direct Comparison: Flattening Effect of N-Adjustment",
      y = "Score Value",
      x = "Sample Size (log scale)"
    ) +
    theme_minimal() +
  ...
  # Panel 2: Spread reduction bar chart
  spread_melt <- melt(spread_wide[, .(hetero_strength, Raw, Adjusted)],
                      id.vars = "hetero_strength",
                      variable.name = "Metric",
                      value.name = "Spread")
  
  p2 <- ggplot(spread_melt, 
               aes(x = as.factor(hetero_strength), y = Spread, fill = Metric)) +
    geom_bar(stat = "identity", position = "dodge", width = 0.7) +
    scale_fill_manual(values = c("Raw" = "#d62728", "Adjusted" = "#2ca02c"),
                      labels = c("Raw Ratio", "Adjusted (c=0.23)")) +
    labs(
      title = "Spread Reduction Across N",
      subtitle = sprintf("Mean reduction: %.1f%%", 
                        mean(spread_wide$reduction_pct)),
      x = expression(paste("Heteroskedasticity Level (", lambda, ")")),
      y = "Spread (Max - Min across N)",
      fill = "Metric"
    ) +
    theme_minimal() +
    theme(legend.position = "bottom")
  
  # Panel 3: Slope comparison
  # Get the actual metric names from the data
  metric_names <- unique(slope_data$Metric)
  raw_name <- metric_names[grepl("Raw", metric_names)]
  adj_name <- metric_names[grepl("Reliability", metric_names)]
  
  # Create named vector for fill colors
  fill_colors <- setNames(c("#d62728", "#2ca02c"), c(raw_name, adj_name))
  
  p3 <- ggplot(slope_data, 
               aes(x = as.factor(hetero_strength), y = abs(slope), fill = Metric)) +
    geom_bar(stat = "identity", position = "dodge", width = 0.7) +
    scale_fill_manual(values = fill_colors,
                      labels = c("Raw Ratio", "Adjusted Score")) +
    labs(
      title = "N-Dependence Slopes",
      subtitle = "Smaller slope = flatter lines = better N-stability",
      x = expression(paste("Heteroskedasticity Level (", lambda, ")")),
      y = "Absolute Slope (change per log N)",
      fill = "Metric"
    ) +
    theme_minimal() +
    theme(legend.position = "bottom")
  
  # Arrange in grid: large top panel, two smaller bottom panels
