...
The matrix implementation directly follows the standard OLS and HC1–HC3 sandwich formulas (White, 1980; MacKinnon and White, 1985; Long and Ervin, 2000).

## 5.2 Numerical Verification
Across all scenarios ($N \in \{20, 50, 100, 1000\}$, $p \in \{1, 4\}$, with homoskedastic, heteroskedastic, and collinear designs), the largest absolute differences are $10^{-9}$ to $10^{-14}$ and relative differences are $10^{-11}$–$10^{-10}$. This is consistent with machine-precision rounding error.

## 5.3 OLS Verification
Manual OLS coefficients and classic covariance match `lm()` and `vcov()` to within machine precision.

**Implication**: The matrix engine is a numerically equivalent drop-in replacement for `lm()` + `vcov()`/`vcovHC()`. This validates its use in the large-scale simulations and calibration steps that follow.
