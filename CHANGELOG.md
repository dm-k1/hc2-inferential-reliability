...
## 3 December 2025 – Remove Backward Compatibility

**Summary**: Removed backward compatibility layer from function calls. Updated function calls to use positional arguments instead of named arguments when the parameter name matches the variable.
...
**Rationale**: Using `N = N` syntax in function calls was maintaining unnecessary backward compatibility after standardizing parameter names.
...
**Summary**: Major refactoring to enforce pipeline compliance. Removed hardcoded theoretical constant 2/(3√N) from metrics, replaced with empirically derived c ≈ 0.25. Centralized all simulation and plotting functions; eliminated ad-hoc inline code from notebooks.
...
- Added error handling checks to NA/Inf detection comments in R/30 and R/40.
...
## 4 December 2025 – Deterministic Failure and Seed Policy Fixes

**Summary**: Resolved deterministic failure policy and seed handling issues.

**Deterministic Failure Compliance (R/10_dgp_and_fits.R)**:
- **Removed `tryCatch`**: The `run_null_simulation_fast()` function previously wrapped `chol()` in `tryCatch` and silently skipped singular matrices. This was replaced with deterministic failure.
...
- **Result**: Core null-calibration engine now has transparent error handling.
...
- **INDEX.md**: Added Part 0 (`00_HC_estimators_validation.Rmd`) to Core Notebooks section for completeness.
...
## 3 December 2025 – Repository Governance and Final Cleanup

**Summary**: Comprehensive cleanup and standardization across the codebase. Removed legacy files, cleaned configuration, verified metric naming consistency, and documented structure completeness.

**Files Removed**:
- `R/05_utils.R` – Legacy file that should have been deleted in previous refactor (functions already migrated to R/10 and R/20)
- `configuration plan.txt` – Planning document archived
...
- ✅ No legacy aliases in production code
- ✅ Three documentation files: `README.md`, `CHANGELOG.md`, `INDEX.md`
- ✅ All R scripts parse correctly
...
- **Rewrote `02_HC2_validation.Rmd`**:
    - Structured into narrative sections covering setup, simulation grid, power analysis, reliability score development, and lookup tables.
    - Developed benchmarking framework against Breusch-Pagan and White tests (later removed from notebooks in favor of focused heteroskedasticity analysis).
    - Demonstrated N-invariance properties of $S_{Adj}$ via regression and mixed-effects models.
...
- **Benchmarking** (Historical note: Benchmarking functions were implemented but later removed from notebooks in favor of direct heteroskedasticity analysis).
